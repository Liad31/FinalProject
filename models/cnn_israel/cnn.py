...
def get_data():
    ...
    #     img = Image.open('./images/'+ im[0] +'.png').convert('RGBA')
    #     arr = np.array(img)[:,:,:-1]
    #     if arr.shape[0] > avg[0] - 2 * std[0] and  arr.shape[1] > avg[1] - 2 * std[1] and arr.shape[0] < avg[0] + 3 * std[0] and arr.shape[1] < avg[1] + 3 * std[1]:
    #         videos.append(arr)
...
def eval(model, data_loader):
    ...
    with torch.no_grad():
        for data, target in data_loader:
            ...
            reals = np.concatenate((reals, target.cpu().detach().flatten()), axis=0)
