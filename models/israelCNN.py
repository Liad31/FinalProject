...
class resnet(nn.Module):
    ...
    def forward(self, x):
        return F.sigmoid(self.model_conv(x))
...
class CNN(nn.Module):
    def __init__(self, ):
        ...
        self.fc_1 = nn.Linear(44944 , FC_SIZE)
...
def get_data():
    ...
    videos = np.load('tagged_images.npy', allow_pickle=True)[:50]
    tags = np.load('images_tags.npy', allow_pickle=True)[:50]
    minsizes= [224,224]# delete it later!
    ...
    for i in range(len(videos)):
        videos[i] = cv2.resize(videos[i], (minsizes[0], minsizes[1]), interpolation=cv2.INTER_CUBIC)
...
def train_epoch(train_loader, model, optimizer):
    ...
    for input, labels in tqdm(train_loader):
        ...
        with torch.set_grad_enabled(True):
            ...
            labels = labels.float()
            outputs = model(input)
            loss = loss_func(torch.unsqueeze(labels,1), outputs.clone())
