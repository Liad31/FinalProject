const DATABASECONNECTION = "mongodb+srv://ourProject:EMGwk59xADuSIIkv@cluster0.lhfaj.mongodb.net/production1?retryWrites=true&w=majority";
const WEEKLY_TAGS_NUM = "50"
const WEKKLY_UPDATE_TIME = "0 0 * * 2"
// const WEKKLY_UPDATE_TIME = "5 * * * * *"
const FEATURE_LIST = ["ביזוי דגל ישראל","דגל ישראל", "חיילים/שוטרים", "חרדים", "הר הבית בצירוף האשטאג לאומני", "חיכוך עם כוחות ביטחון", "שהיד", "דגל פלסטין", "שמחה לאיד ליהודים", "כאפייה פלסטינית"]


module.exports = {
    DATABASECONNECTION,
    WEEKLY_TAGS_NUM,
    WEKKLY_UPDATE_TIME,
    FEATURE_LIST
};